...
class GenomeDatabase(object):
    ...
    def AddManyFastaGenomes(self, batchfile, checkM_file, modify_genome_list_id=None,
        ...
        try:
            ...
            if len(list(fasta_paths_to_copy.keys())) > 0:
                username = None
                ...
                try:
                    for (genome_id, details) in list(fasta_paths_to_copy.items()):
    ...
    def DeleteGenomes(self, batchfile=None, external_ids=None):
        try:
            ...
            try:
                ...
                self.ReportWarning("Exception was raised when deleting genomes. Some orphans may remain. Exception message: %s" % str(e))
    ...
    def ExternalGenomeIdsToGenomeIds(self, external_ids):
        try:
            ...
            if len(list(map_sources_to_ids.keys())):
                ...
                cur.executemany(query, [(x,) for x in list(map_sources_to_ids.keys())])
            else:
            ...
            for (query_prefix,) in cur:
                missing_genome_sources[query_prefix] = list(map_sources_to_ids[query_prefix].values())
            ...
            for source_prefix in list(map_sources_to_ids.keys()):
                ...
                cur.executemany(query, [(x,) for x in list(map_sources_to_ids[source_prefix].keys())])
    ...
    def AddMarkers(self, batchfile, modify_marker_set_id=None, new_marker_set_name=None,
        ...
        try:
            ...
            if len(list(hmm_paths_to_copy.keys())) > 0:
                username = None
                if self.currentUser.isRootUser():
                    username = self.currentUser.getElevatedFromUsername()
                else:
                ...
                try:
                    for (marker_id, details) in list(hmm_paths_to_copy.items()):
    ...
    def AddMarkerWorking(self, cur, marker_file_path, name, desc, marker_set_id=None, force_overwrite=False,
        ...
        try:
            try:
                marker_fh = open(marker_file_path, "r")
            except:
    ...
    def DeleteMarkers(self, batchfile=None, external_ids=None):
        try:
            ...
            try:
                ...
                self.ReportWarning("Exception was raised when deleting markers. Some orphans may remain. Exception message: %s" % str(e))
    ...
    def ExternalMarkerIdsToMarkerIds(self, external_ids):
        try:
            ...
            if len(list(map_databases_to_ids.keys())):
                cur.execute("CREATE TEMP TABLE %s (prefix text)" % (temp_table_name,) )
                query = "INSERT INTO {0} (prefix) VALUES (%s)".format(temp_table_name)
                cur.executemany(query, [(x,) for x in list(map_databases_to_ids.keys())])
            else:
            ...
            for (query_prefix,) in cur:
                missing_marker_sources[query_prefix] = list(map_databases_to_ids[query_prefix].values())
            ...
            for database_prefix in list(map_databases_to_ids.keys()):
                ...
                cur.executemany(query, [(x,) for x in list(map_databases_to_ids[database_prefix].keys())])
    ...
    def MakeTreeData(self, marker_ids, genome_ids, directory, prefix, profile=None, config_dict=None, build_tree=True):
        try:
            ...
            if uncalculated_marker_count > 0:
                print("%i genomes contain %i uncalculated markers." % (len(list(uncalculated_marker_dict.keys())), uncalculated_marker_count))
            ...
            uncalculated_marker_list = list(uncalculated_marker_dict.items())
            chunk_size = 500
            ...
            for (index, this_chunk) in enumerate(uncalculated_marker_dict_chunks):
                ...
                while True:
                    for (genome_id, async_result) in list(genome_id_to_async_result.items()):
                ...
                while True:
                    ...
                    for i in range(0, len(all_marker_async_results)):
                        ...
                        for (marker_id, async_result) in list(genome_marker_async_results['results'].items()):
                        ...
                        if all_genome_markers_complete:
                            ...
                            updated_marker_ids = list(genome_marker_async_results['results'].keys())
                            ...
                            cur.executemany("SELECT upsert_aligned_markers(%s, %s, %s, %s, %s)", list(zip(
    ...
    def ViewMarkerSetsContents(self, marker_set_ids):
        try:
            ...
            marker_ids = self.GetMarkerIdListFromMarkerSetId(marker_set_ids[0])
