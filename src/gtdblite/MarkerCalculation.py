...
def RunProdigalOnGenomeFasta(fasta_path):
    try:
        ...
        out_fh = open(modified_genes_filepath, 'w')
        genes_fh = open(os.path.join(prodigal_dir, 'genes.faa'), 'r')
...
def CalculateBestMarkerOnProdigalDir(output_prefix, marker_hmm_file, prodigal_dir):
    ...
    model = next(hmmmodelparser.HmmModelParser(marker_hmm_file).parse())
...
def GetOrderedVettedHitsInHMMSearchResult(model, hmmsearch_result_file):
    ...
    while True:
        ...
        if not resultsParser.vetHit(model, hit):
            continue
        hits.append(hit)
    return sorted(hits, key=lambda x: x.full_score)
